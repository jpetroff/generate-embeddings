...
class SimpleIngestComponent(BaseIngestComponentWithIndex):
    ...
    def ingest(self, file_name: str, file_data: Path) -> list[Document]:
        ...
        documents = Reader.transform_file_into_documents(file_name, file_data)
        logger.info(
    ...
    def bulk_ingest(self, files: list[tuple[str, Path]]) -> list[Document]:
        ...
        for file_name, file_data in files:
            documents = Reader.transform_file_into_documents(
                file_name, file_data
            )
    ...
    def _save_docs(self, documents: list[Document]) -> list[Document]:
        ...
        with self._index_thread_lock:
            for document in documents:
                # new_doc = IngestedDoc.process_node(document)
                new_doc = document
                self._index.insert(new_doc, show_progress=True)
...
class ParallelizedIngestComponent(BaseIngestComponentWithIndex):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._callback = kwargs['callback'] or None

    def ingest(self, file_name: str, file_data: Path, **kwargs: Any) -> list[Document]:
        print(f"Reading [purple]{file_name}[/]")
        logger.info("Ingesting file_name=%s", file_name)
        ...
        documents = self._file_to_documents_work_pool.apply(
            Reader.transform_file_into_documents, (file_name, file_data)
        )

        process_file: bool = True

        if(callable(self._callback) ):
            process_file = self._callback(
                file=file_name,
                count=len(documents),
                # message=f"[purple]{file_name}[/] → {len(documents)} docs",
                documents=[d.get_doc_id() for d in documents],
                type="compare",
                **(self._callback.__kwdefaults__ or {})
                )
        ...
        print(f"{len(documents)} docs ← {file_name}")

        if process_file == False:
            print(f"[[bold]✓[/]] Skipped [purple]{file_name}[/]")
            return []

        logger.debug("Saving the documents in the index and doc store")
        result_documents = self._save_docs(documents, file_name)
        return result_documents

    def bulk_ingest(self, files: list[tuple[str, Path]], **kwargs: Any) -> list[Document]:
    ...
    def _save_docs(self, documents: list[Document], file_name: str) -> list[Document]:
        logger.debug("Transforming count=%s documents into nodes", len(documents))
